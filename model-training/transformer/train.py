...
print(f"made dataset, this took {time.time() - dataset_start:.2f}s", file=sys.stderr)
...
net_state = TrainState.create(
    apply_fn=configs['net'].apply, 
    params=init_params, 
    key=jrandom.PRNGKey(configs['dropout_init_seed']),
    tx=configs['optimizer']
)
