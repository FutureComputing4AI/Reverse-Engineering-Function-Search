...
class AssemblageFunctionsDataset(Dataset):
    ...
    def __init__(self, database_path='data.sqlite', binaries_base_path='dataset', ids_to_select=None,
        ...
        select_fns_base_query = "SELECT {} \
                            FROM binaries b JOIN functions f ON b.id==f.binary_id \
                            ORDER BY f.id"
        
        print("Executing Names query")
        names = self.cursor.execute(select_fns_base_query.format("f.name")).fetchall()
        print("Executing Family IDs query")
        family_ids = self.cursor.execute(select_fns_base_query.format("b.family_id")).fetchall()
