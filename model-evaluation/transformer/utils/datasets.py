...
class BytesCSVDataset(Dataset):
    """
        PyTorch dataset for functions extracted to ghidra, with their addresses saved in CSVs,
        aka the output of ExtractFunctionBytesToCSV.java. This is designed for systems that can't 
        efficiently handle each function extracted to its own file.

        Arguments:
            path_to_csvs: Path to a folder containing the csv outputs (and only the csv outputs), 
                arranged in whatever directory structure is most efficient for your system.
    """

    def __init__(self, path_to_csvs):

        self.base_path = Path(path_to_csvs)
        
        self.files = list(self.base_path.rglob("*.csv"))
        columns = ["hash", "path", "name", "start_rva", "end_rva"]

        self.df = pd.concat([pd.read_csv(os.path.join(self.base_path, file), names=columns) for file in self.files])
        self.labels = torch.tensor(pd.Categorical(self.df["name"]).codes)

    def _get_bytes(self, index):
        _, file_path, _, start_rva, end_rva = self.df.iloc[index]

        print(file_path)

        pe = pefile.PE(os.path.join(self.base_path, file_path), fast_load=True)
        func_array = extract_function(pe, (start_rva, end_rva))
        pe.close()
    ...
    def __getitem__(self, idx):
        return self._get_bytes(idx), self.labels[idx]
...
class MotifDataset(GhidraFunctionsDatasetBase):
    ...
    def _getlabel(self, fn_file_name):
        new_file_hash = fn_file_name.split('\\')[0].split('_')[0]
...
class LargeGhidraFunctionsDataset(GhidraFunctionsDatasetBase):
    """
    PyTorch dataset for functions extracted by Ghidra and ExtractFunctionBytes.java using
    Large Dataset Mode, where name=label

    path_to_functions: path to the folder where the function files from running
                        ExtractFunctionBytes.java were stored
    """
    def __init__(self, path_to_functions):
        super().__init__(path_to_functions)

        self.data = []
        for root, _, files in os.walk(self.fn_base_path):
            filtered_root = root.replace(self.fn_base_path + '/', "")
            for file in files:
                self.data.append(os.path.join(filtered_root, file))
                
        names = [file_name.split('\\')[0] for file_name in self.data]
    ...
    def _getlabel(self, fn_file_name):
        function_name = fn_file_name.split('\\')[0]
        return self.names_to_labels[function_name]
